...
/**
 * Reads from {@code stdin} and directs to writes to a {@link org.slf4j.Logger} at the
...
public class LogFromStdin {
     ...
     *
     * @param levelRequested String value of a {@link LogAtLevel} Enum. If {@code null} is
     *                       given, {@code INFO} is used.
     */
    public void runLogger(String levelRequested) {
        LineLogger.LogAtLevel level = LineLogger.LogAtLevel.INFO;
        if (levelRequested != null) {
            level = LineLogger.LogAtLevel.valueOf(levelRequested);
        }
